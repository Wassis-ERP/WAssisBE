...
| Testes negativos PostgreSQL/HTTP | Executado localmente | `PostgresAdministrationTests` semeia dois tenants e testa conflitos, datas, auto-inativação, último Master concorrente, permissão persistida, métricas e auditoria. O driver entrega timestamps/datas como `DateTime`; o repositório converte datas de vigência para `DateOnly` antes de expor o contrato HTTP. |
...
O teste de integração inicia um PostgreSQL descartável via Testcontainers, executa o migration gate singleton e usa HTTP real pelo `WebApplicationFactory`. Nesta estação, o SDK .NET 8 foi instalado temporariamente; o build Release local passou com 0 avisos/erros e a suíte completa passou **99/99** no Debian WSL com Docker. O Windows continuou sem Docker Engine disponível. Os avisos de recursos de idioma do SDK Linux foram evitados com `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1` e `NoWarn=NETSDK1188`; esse ajuste do ambiente não altera código de produção. Antes de promover HML/PRD, executar também o ensaio de migrations idempotentes contra o legado documentado e registrar saída, SHA e plano de rollback. A escolha de Auth0 foi confirmada, mas o usuário informou que ainda não existe tenant HML; esse gate depende de provisionamento e acesso administrativo limitado sem envio de segredo no chat.
