...
public sealed class CreateOpportunityCommandValidator : AbstractValidator<CreateOpportunityCommand>
{
    public CreateOpportunityCommandValidator()
    {
        ...
        RuleFor(x => x).Must(x => !string.Equals(x.Status, "won", StringComparison.OrdinalIgnoreCase) || x.WonAtUtc.HasValue)
            .WithMessage("Won opportunities must include WonAtUtc.");
        RuleFor(x => x).Must(x => !string.Equals(x.Status, "lost", StringComparison.OrdinalIgnoreCase) || x.LostAtUtc.HasValue)
            .WithMessage("Lost opportunities must include LostAtUtc.");
