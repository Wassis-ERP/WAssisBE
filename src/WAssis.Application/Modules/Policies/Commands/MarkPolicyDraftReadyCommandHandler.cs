...
public sealed class MarkPolicyDraftReadyCommandHandler(
    IPolicyDraftRepository repository,
    IAuditTrailWriter auditTrailWriter)
    : IRequestHandler<MarkPolicyDraftReadyCommand, Result<PolicyDraftDto>>
{
    public async Task<Result<PolicyDraftDto>> Handle(MarkPolicyDraftReadyCommand request, CancellationToken cancellationToken)
    {
        ...
        draft.MarkReadyForIssuance(request.Notes);
        await repository.SaveChangesAsync(cancellationToken);

        await auditTrailWriter.WriteAsync(
            draft.CorrelationId,
            "Policies",
            "MarkReadyForIssuance",
